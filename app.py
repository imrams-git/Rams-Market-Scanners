...
selected_script = st.sidebar.selectbox(
    ...
    ["India Volume Matching", 
     ...
     "India SMA200",
     "US SMA200",
     "India SMA200 And Matching",
     "US SMA200 And Matching",
	 "India High-Prob EMA Breakout",
     "US High-Prob EMA Breakout",
     "M US stock Analyser",
...
# --- SCRIPT 5: INDIA SMA200 And MATCHING ---
if selected_script == "India SMA200 And Matching":
    st.subheader("📈 Stocks bouncing off SMA200 with Volume Scanner (India)")
    st.markdown("Scans for stocks bouncing off 200 SMA and having volume momentum candles and checks if support/resistance levels remain unbroken.")
    n_bars = st.slider("Lookback Bars", min_value=20, max_value=200, value=100)
...
# --- SCRIPT 6: US SMA200 And MATCHING ---
if selected_script == "US SMA200 And Matching":
    st.subheader("📈 Stocks bouncing off SMA200 with Volume Scanner (India)")
    st.markdown("Scans for stocks bouncing off 200 SMA and having volume momentum candles and checks if support/resistance levels remain unbroken.")
    n_bars = st.slider("Lookback Bars", min_value=20, max_value=200, value=100)
...
# --- SCRIPT 7: INDIA EMA BREAKOUT ---
elif selected_script == "India High-Prob EMA Breakout":
    st.subheader("📈 High-Probability Breakout Screener (India)")
    st.markdown("Scans for EMA Crossovers, Volatility Contraction (Coil), and RSI Momentum.")

    if st.button("Execute EMA Breakout Scan", type="primary"):
        with st.spinner("Initializing EMA calculations..."):
            india_ema = importlib.import_module("Sl_India_EMA")
            
            result_df = india_ema.screen_stocks(INDIA_SYMBOLS)

            if result_df is not None and not result_df.empty:
                st.success(f"Successfully found {len(result_df)} high-probability candidates!")
                st.dataframe(result_df, hide_index=True, use_container_width=True)
            else:
                st.warning("No stocks currently match the strict breakout criteria.")


# --- SCRIPT 8: US EMA BREAKOUT ---
elif selected_script == "US High-Prob EMA Breakout":
    st.subheader("📈 High-Probability Breakout Screener (US)")
    st.markdown("Scans for EMA Crossovers, Volatility Contraction (Coil), and RSI Momentum.")

    if st.button("Execute EMA Breakout Scan", type="primary"):
        with st.spinner("Initializing EMA calculations..."):
            us_ema = importlib.import_module("Sl_US_EMA")

            result_df = us_ema.screen_stocks(US_SYMBOLS)
...
# --- SCRIPT 7: M US stock Analyser ---
    ...
    us_analyzer = importlib.import_module("M_US_stock_analyzer")
    selected_sector = st.sidebar.selectbox("Select Universe / Sector Group", list(us_analyzer.US_SECTORS.keys()))
    ...
    if st.button("🚀 Run Analysis", type="primary"):
        ...
        df = us_analyzer.US_run_analysis(sector=selected_sector, top_n=top_n, progress_callback=update_streamlit_progress)
...
# --- SCRIPT 6: M India stock Analyser ---
    ...
    india_analyzer = importlib.import_module("M_India_stock_analyzer")
    selected_sector = st.sidebar.selectbox("Select Universe / Sector Group", list(india_analyzer.INDIAN_SECTORS.keys()))
    top_n = st.sidebar.slider("Show Top N Stocks", min_value=5, max_value=30, value=10)
    ...
    if st.button("🚀 Run Analysis", type="primary"):
        ...
        df = india_analyzer.India_run_analysis(sector=selected_sector, top_n=top_n, progress_callback=update_streamlit_progress)
