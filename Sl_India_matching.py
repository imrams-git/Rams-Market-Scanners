...
class VolumeAlertChecker:
    ...
    def get_time_adjusted_rvol(self, current_vol, avg_vol):
        ...
        if now_local < market_open: return 0
        if now_local >= market_close: return current_vol / avg_vol if avg_vol > 0 else 0
    ...
    def run(self, symbols: List[str], n_bars: int = 100):
        ...
        filepath = os.path.join(output_dir, f"India_Matching_{timestamp_str}.csv")
